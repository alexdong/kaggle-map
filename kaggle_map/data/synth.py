...
class SynthDataGenerator:
    ...
    def call_llm(self, prompt: str, retries: int = 3) -> str:
        ...
        # This should never be reached due to the return in the else block
        return ""
    ...
    def _should_skip_row(self, current_row: int) -> bool:
    ...
    def _update_progress(
        ...
        """Update progress tracking and save checkpoints."""
        ...
        if current_row % CHECKPOINT_INTERVAL == 0:
            ...
            f.flush()
            logger.info(
                f"Progress: {current_row:,}/{total_output_rows:,} rows ({current_row / total_output_rows * 100:.1f}%)"
            )
    ...
    def _process_rows(
        ...
        for orig_row in original_rows:
            for expansion_idx in range(EXPANSIONS_PER_ROW):
                ...
                if self._should_skip_row(current_row):
                    continue

                self._log_debug_info(current_row, orig_row, expansion_idx)
                new_row = self._create_synthetic_row(orig_row, expansion_idx)

                writer.writerow(new_row)
                f.flush()

                self._update_progress(current_row, task, progress, total_output_rows, f)

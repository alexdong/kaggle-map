...
def test_prompt_template_exists() -> None:
    ...
    assert template_path.exists(), "baseline.j2 template file should exist"


def test_template_renders_all_parameters() -> None:
    ...
    rendered = template.render(mc_answer=mc_answer, student_explanation=student_explanation, predictions=predictions)
...
def test_build_reranking_prompt_integration() -> None:
    ...
    request = RerankingRequest(evaluation_row=evaluation_row, candidate_predictions=predictions)
...
def test_template_handles_special_characters() -> None:

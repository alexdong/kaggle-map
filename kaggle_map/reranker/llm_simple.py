...
# Cache for label normalization
LABEL_CACHE_SIZE = 1024
...
def build_reranking_prompt(request: RerankingRequest) -> PromptTemplate:
    ...
    predictions_text = "\n".join(
        f"{i+1}. {pred!s}"
        for i, pred in enumerate(request.candidate_predictions)
    )
...
def process_dataframe_simple(
    ...
    # Required columns
    required = ["QuestionText", "MC_Answer", "StudentExplanation",
                "top_3_predictions_formatted", "Category"]

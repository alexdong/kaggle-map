...
@pytest.fixture
def sample_generation(sample_candidates: list[PromptCandidate], sample_evaluations: list[EvaluationResult]) -> Generation:
...
@pytest.mark.parametrize("scores,threshold,window,expected,description", [
...
def test_check_convergence(scores: list[float], threshold: float, window: int, expected: bool, description: str) -> None:
...
@pytest.mark.parametrize("top_percentage,expected_count,expected_first,expected_second", [
...
def test_run_generation(
    ...
    # Verify results
    assert generation.generation_id == sample_evolution_context.next_generation_id, f"Generation ID should match context's next generation ID"
...
def test_evolve_prompts(
    ...
    assert mock_storage.save_generation.call_count >= max_generations, f"Should save at least {max_generations} generations"

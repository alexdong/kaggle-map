...
def _should_skip_file(filename: str) -> bool:
    ...
    if should_skip:
        logger.debug(f"Skipping file '{filename}': {'private/internal file' if filename.startswith('_') else 'base/init file'}")
    return should_skip
...
def _log_discovery_summary(processed_files: int, skipped_files: int, failed_imports: int, successful_strategies: int) -> None:
...
def get_strategy(name: str) -> type[Strategy]:
    ...
    suggestions = [
        available for available in available_names
